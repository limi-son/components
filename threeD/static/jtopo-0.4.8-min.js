...
function (a) {
    function b(a) {
        // 此处的a是stage对象，一个抽象的舞台对象,对应一个Canvas和多个场景对象(Scene)
        // a.childs为场景对象数组
        return {
        ...
        this.initialize = function (c) {
			...
			if (null != b && "" != b) {
				...
				if(bs.length>1){
					for(var j= 0;j<bs.length;j++){
                        ...
                        var positionStr = this.textPosition;
                        if(this.textPosition.indexOf('_Center')!==-1){
                            x = e.x+(c-a.measureText(bs[j]).width)/2
                        }
                        else if(
                            // container中内右边
                            ( positionStr.indexOf('Inner_')!==-1 && positionStr.indexOf('_Right')!==-1 ) ||
                            // container中左外
                            ( positionStr.indexOf('Inner_')===-1 &&  positionStr.indexOf('_Left')!==-1 )
                        ){
                            x = e.x+(c-a.measureText(bs[j]).width)
                        }else{
                            x = e.x;
                        }

                        if(positionStr.indexOf('Inner_Bottom_')!==-1){
                            y = e.y+j*d-(bs.length-1)*d
                        }else if(positionStr.indexOf('Inner_Top_')!==-1){
                            y = e.y+j*d;
                        }else if(
                            positionStr.indexOf('_Middle_')!==-1 ||
                            positionStr.indexOf('Middle_')!==-1
                        ){
                            // 居中时 y轴计算
                            y = e.y + j*d-(bs.length-1)*d/2;
                        }else if(positionStr.indexOf('Top_')!==-1){
                            y = e.y-(bs.length-j-1)*d;
                        }else{
                            y = e.y;
                        }
                        a.fillText(bs[j],x,y);
            ...
            d = {
                ...
                y: this.height / 2 + c
                        ...
                        : "Inner_Top_Center" == a ? d = {
                            x: -this.width / 2 + (this.width - b) / 2,
                            y: -this.height / 2 + c,
                        } 
                        : "Inner_Bottom_Right" == a ? d = {
                            x: this.width / 2,
                            y: this.height / 2 ,
                        } 
                        : "Inner_Bottom_Left" == a ? d = {
                            x: -this.width / 2 - b,
                            y: this.height / 2 ,
                        } 
                        :"Inner_Bottom_Center" == a ? d = {
                            x: -this.width / 2 + (this.width - b) / 2,
                            y: this.height / 2 ,
                        } 
                        // 上面为增加文字布局
...
function (a) {
    function b(c) {
        this.initialize = function (c) {
            ...
            // 文字布局code 在此可以自定义增加布局   container文字text布局？？？
            var d = null;
            return null == a || 
            // 下面为增加文字布局  c为文字内容height
            "Inner_Top_Left" == a ? d = {
            ...
            : "Inner_Middle_Right" == a ? d = {
                x: this.x + this.width - b,
                y: this.y + this.height / 2 + c / 2
            } 
            : "Inner_Middle_Left" == a ? d = {
...
function (a) {
    ...
    function u(a, b,) {
        return null == a && (a = 0), null == b && (b = 0), function (c) {
            ...
            var 
            flag = true,                         // 判断符号
            upTextSize = 16,                     // i.textPosition=='Top_Middle'
            downTextSize = 16,                   // i.textPosition=='Bottom_Center'
            preRowYLocation = [],                // 存放上一行的所有元素左下角y坐标  
            preRowY=c.getBound().top+c.borderWidth;            // 存放上一行的下边界y   Math.max.apply(null, preRowYLocation) 
            ...
            if (!(d.length <= 0)) {
                for (var e = c.getBound(), f = e.left, g = e.top, h = 0; h < d.length; h++) {
                    ...
                    // 未给容器命名 或 命名为空时
                    if(i.text===undefined||String(i.text).length==0) {downTextSize=0,upTextSize=0}
                    else{
                        i.textPosition=='Bottom_Center'&&String(i.text).length>0? downTextSize=16:downTextSize=0;
                        i.textPosition=='Top_Middle'&&String(i.text).length>0? upTextSize=16:upTextSize=0;
                    }
                    
                    if( f + i.width <= e.right && flag ){
                        preRowYLocation.push(upTextSize + preRowY + b + i.height + downTextSize) ;
                    }else{
                        flag = false;
                    }
                    ...
                    if(!flag){
                        f = e.left;
                        preRowY=Math.min.apply(null, preRowYLocation);
                        f += preRowYLocation.indexOf(preRowY)*(a + i.width);
                        preRowYLocation[preRowYLocation.indexOf(preRowY)] = upTextSize + preRowY + b + i.height + downTextSize;
                        g = preRowY;
                    }
            ...
            // preRowY = Math.max.apply(null, preRowYLocation);
            // c.setSize(c.getBound().width,preRowY-e.top);
            // c.width = c.getBound().width;
            // c.height = preRowY-e.top;

            preRowY = Math.max.apply(null, preRowYLocation)-b;  //减去多计算的垂直间距局
            if(c.getBound().height<preRowY-c.getBound().top || !c.getBound().height) {
                c.setSize(c.getBound().width,preRowY-c.getBound().top);
                c.width = c.getBound().width;
                c.height = preRowY-c.getBound().top;
            }
        }
    }
        ...
        return null == a && (a = 0), null == b && (b = 0), function (c) {
            ...
            if (!(d.length <= 0)) {
                for (var e = c.getBound(), f = e.left, g = e.top, h = 0; h < d.length; h++) {
                    ...
                    f + i.width > e.right && (f = e.left,preRowY=Math.max.apply(null, preRowYLocation), preRowYLocation=[],preRowYLocation.push(upTextSize + preRowY + b + i.height + downTextSize),g = preRowY), i.setLocation(f, g), f += a + i.width
                }
            }
            
            preRowY = Math.max.apply(null, preRowYLocation)-b;   // 减去多出来的垂直间距
            if(c.getBound().height<preRowY-c.getBound().top || !c.getBound().height) c.setSize(c.getBound().width,preRowY-c.getBound().top);
        }
    }
    ...
    a.layout = a.Layout = {
        ...
        getRootNodes: g,
        GridLayout: d,               // 网格布局
        MinSpaceFlowLayout: u,       // 自定义最优空间流式布局
        FlowLayout: e,               // 等行高流式布局-已改善code
        AutoBoundLayout: f,          // 自由边界布局
        CircleLayout: l,             // 环形布局
        TreeLayout: k,               // 树形布局
        getNodesCenter: b,
        circleLayoutNodes: c
